...
function applyOccupantsChange(building, number, occupants){
  ...
  var change = {
    ...
    roomId: id, building: building, number: number, occupants: occupants
  };
...
function renderDashboard(){
  ...
  app.innerHTML =
  '<div class="hero-banner"><div class="txt">🌾 INP-HB Centre — Évangélisation 2026</div></div>' +
...
function exportCsv(){
  var rows = [["Bâtiment","Chambre","Occupants","Étudiant 1","Étudiant 2","Statut","Dernière mise à jour"]];
  BUILDINGS.forEach(function(b){
    var rc = roomCount(b);
    for(var i=1;i<=rc;i++){
      var room = getRoom(b,i);
      ...
      rows.push([b, b+"-"+i, occ, room.s1.r?"Oui":"Non", occ===2?(room.s2.r?"Oui":"Non"):"N/A", c+"/"+occ, lastAt?new Date(lastAt).toLocaleString("fr-FR"):"-"]);
    }
  });
